...
bool Object::loadOBJ(const char* path) {
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			vertices.push_back(tempVertices[vertexIndex[0] - 1]);
			vertices.push_back(tempVertices[vertexIndex[1] - 1]);
			vertices.push_back(tempVertices[vertexIndex[2] - 1]);
...
void Object::draw(glm::mat4 View, glm::mat4 Proj) {
	...
	glBindVertexArray(0);
	glActiveTexture(GL_TEXTURE1);  // 또는 다른 텍스처 유닛을 선택
	glBindTexture(GL_TEXTURE_2D, 0);
	glActiveTexture(GL_TEXTURE0);  // 텍스처 유닛 0을 선택하여 비활성화
	glBindTexture(GL_TEXTURE_2D, 0);
